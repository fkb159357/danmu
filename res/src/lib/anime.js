...
Anime.Jinguanyu = function(w_start, h_start){
    ...
    function create(w_start, h_start){
        ...
        var src = ( (-1 == ['localhost', '127.0.0.1', 'danmu.me', 'miku.us'].indexOf(location.hostname)) ? 'http://miku.us' : '') + '/res/biz/danmu/jinguanyu.gif';//处于danmu项目中时，不加http://miku.us/
...
(function(){
    var max = parseInt((location.href.match(/[\?\&\#\;]jinguanyu=(\d+$)/) || [,5])[1]);//最少5只
    for(var i = 0; i < max; i++) {
