...
class DIUrlShell {
     ...
     * 默认命令
     */
    static $_default_shell = 'danmu/start';
    ...
    static function doshell(){
        return array(
            ...
            'Danmu' => array(
            	'gen' => 0, //根据弹幕、视频地址，生成UBB代码到json，用于播放，gen需要调用model层的返回结果，再生成json
            	'search' => 0, //列出弹幕服务器当前收录的弹幕视频到分页,支持三种调用方式,可搜索
            	'play' => 1, //播放页，需要参数-弹幕id
            	'mod' => 0, //更新弹幕信息，需要POST一个参数数组
            	'del' => 1, //删除弹幕，返回json
            	'okasii' => 0,//奇怪功能
            ),
            //这里有必要显式声明，兼容LINUX/UNIX文件系统中大小写敏感的问题，这样就可用rootcommon/func正确定位RootCommon.do.php了
            'RootCommon' => array(),
            'OauthQq' => array(),
        );
    }
